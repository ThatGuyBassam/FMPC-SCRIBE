import sys, json, os, time, shutil, subprocess, requests
...
# ─── OLLAMA WITH RETRY ─────────────────────────────────────────────
def ollama_with_retry(prompt, timeout=150, retries=3, backoff=5):
    """Call Ollama with automatic retry on failure. Falls back gracefully."""
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt < retries:
                print(f"[PROCESSOR]   Ollama attempt {attempt}/{retries} failed ({e}). Retrying in {backoff}s...")
                time.sleep(backoff)
            else:
                print(f"[PROCESSOR]   Ollama failed after {retries} attempts ({e}). Using original.")
                return None
...
def clean_transcript_chunked(transcript_text):
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        cleaned = ollama_with_retry(prompt)
        if cleaned:
...
def filter_transcript_chunked(transcript_text):
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        filtered = ollama_with_retry(prompt)
        if filtered:
...
def main():
    ...
    if result_ffmpeg.returncode != 0:
        ...
        print(f"[PROCESSOR] WARNING: Opus archiving failed — original audio NOT deleted: {file_path}")
    else:
        ...
        # Only delete original if Opus archive was successfully created
        os.remove(file_path)
        print(f"[PROCESSOR] Original audio deleted after successful archive.")

    # Release Ollama VRAM immediately so next transcriber run doesn't OOM
    try:
        ...
        print("[PROCESSOR] Ollama model unloaded from VRAM.")
    except Exception:
        pass  # Non-critical — Ollama will unload on its own after 5 min
