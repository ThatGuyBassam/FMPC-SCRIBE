...
- **Topic field** — optional. Specify a subject to focus the questions or leave blank for random coverage
- **TOR scoring** — all-or-nothing. Must select every correct answer and no wrong ones to score the point
...
All saved QCM sessions listed with discipline, topic, question count, and date. Click **Pratiquer** to redo any session with full TOR scoring and corrections. Delete with ✕. Stored in `qcm_bank.json`.
...
- Score card: X/N, percentage, RÉUSSI (≥50%) or ÉCHOUÉ
- Full correction with per-proposition colour coding and explanation from professor material
...
Full pipeline operational. Transcription stable on 2-hour lectures. Study interface on FastAPI — RAG assistant, 15-question QCM with course selector and bank, real exam practice with TOR scoring and professor-grounded corrections. Raspberry Pi assembly in progress.
